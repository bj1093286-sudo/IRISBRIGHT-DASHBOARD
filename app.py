...
GID_MAP  = {"agent":"0","phone":"1","chat":"2","board":"3"}
...
# PERIOD UTILITIES
# ─────────────────────────────────────────────
def get_period_col(unit: str) -> str:
    return {"일별":"일자","주별":"주차","월별":"월"}.get(unit,"일자")

def assign_period_cols(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    df["일자_d"]  = df["일자"].dt.date
    df["주차"]    = df["일자"].dt.to_period("W").apply(lambda p: str(p.start_time.date()))
    df["월"]      = df["일자"].dt.to_period("M").apply(lambda p: p.strftime("%Y-%m"))
    return df

def get_prev_period(unit: str, current_label: str):
    """전기 라벨 반환"""
    try:
        if unit == "일별":
...
def get_yoy_period(unit: str, current_label: str):
    """전년 동기 라벨 반환"""
    try:
        if unit == "일별":
            d = datetime.strptime(str(current_label), "%Y-%m-%d")
            # 같은 요일 기준 — 52주 전
            yoy = d - timedelta(weeks=52)
            return yoy.strftime("%Y-%m-%d")
        elif unit == "주별":
            d = datetime.strptime(str(current_label), "%Y-%m-%d")
            return (d - timedelta(weeks=52)).strftime("%Y-%m-%d")
        elif unit == "월별":
            d = datetime.strptime(str(current_label) + "-01", "%Y-%m-%d")
            yoy = d.replace(year=d.year - 1)
            return yoy.strftime("%Y-%m")
    except:
        return None

def get_chart_range(unit: str, max_months: int = 3):
    """추이 차트 표시 시작 기준일 반환"""
    today = datetime.today()
    if unit == "일별":
        return today - timedelta(days=89)       # 최근 90일
    elif unit == "주별":
        return today - timedelta(weeks=12)      # 최근 13주
    elif unit == "월별":
        months = max_months                     # 슬라이더 값
        return (today.replace(day=1) -
                timedelta(days=1)).replace(day=1) - timedelta(days=30*(months-1))
    return today - timedelta(days=89)

def build_period_series(df: pd.DataFrame, unit: str, val_col: str = None,
                        agg: str = "count") -> pd.Series:
    """단위별 집계 시리즈 반환"""
    pc = get_period_col(unit)
    if val_col and agg != "count":
        return df.groupby(pc)[val_col].agg(agg)
    return df.groupby(pc).size()
...
def kpi(label, value, sub="", delta=None, delta_label="전기대비",
        reverse=False, color="#3b82f6"):
    dh = ""
    if delta is not None:
        if delta > 0:
            ...
            dh   = f'<div class="kpi-delta-neu">- 변동없음</div>'
...
def response_rate(df):
    ...
    return round((~df["미응대"]).sum() / total * 100, 1)

# ─────────────────────────────────────────────
# PERIOD KPI BUILDER
# ─────────────────────────────────────────────
def get_period_kpi(df, unit: str, val_col=None, agg="count",
                   responded_only=False):
    """
    현재 기간 최신값 / 전기값 / 전년값 반환
    Returns: (current_val, prev_val, yoy_val, current_label)
    """
    ...
    prev_label  = get_prev_period(unit, cur_label)
    yoy_label   = get_yoy_period(unit, cur_label)
    prev_val    = series.get(prev_label, None)
    yoy_val     = series.get(yoy_label, None)
    return cur_val, prev_val, yoy_val, cur_label

# ─────────────────────────────────────────────
# TREND CHART BUILDER
# ─────────────────────────────────────────────
def trend_chart(df_dict: dict, unit: str, val_col=None,
                agg="count", responded_only=False,
                y_label="건수", h=320, month_range=3):
    """
    df_dict = {"채널명": df, ...}
    """
    ...
    for name, df in df_dict.items():
        ...
        traces.append((name, s))

    if not traces:
        st.info("표시할 데이터가 없습니다.")
        return

    cmap  = make_color_map([t[0] for t in traces])
    fig   = go.Figure()
    for name, s in traces:
        fig.add_trace(go.Scatter(
            x=s[pc], y=s[y_label],
            mode="lines+markers", name=name,
            line=dict(color=cmap.get(name,"#888"), width=2.5),
            marker=dict(size=7),
            fill="tozeroy",
            fillcolor=cmap.get(name,"#888").replace(")", ",0.07)").replace("rgb","rgba")
                       if "rgb" in cmap.get(name,"") else
                       cmap.get(name,"#888") + "12",
        ))
    fig.update_layout(
...
def bar_trend_chart(df_dict: dict, unit: str, val_col=None,
                    agg="count", y_label="건수",
                    h=300, month_range=3):
    cutoff = get_chart_range(unit, month_range)
    pc     = get_period_col(unit)
    rows   = []
...
def delta_row(cur, prev, yoy, unit, reverse=False):
    ...
    parts.append(badge(p_delta, prev_lbl, reverse))
    parts.append(badge(y_delta, "전년동기", reverse))
...
def page_overview(phone, chat, board, unit, month_range):
    ...
    total = len(phone) + len(chat) + len(board)
    ...
    with c2:
        kpi("전화 인입 (최근 기간)",
            ...
            delta_label={"일별":"전일","주별":"전주","월별":"전월"}.get(unit,"전기"),
            color="#3b82f6")
    with c3:
        kpi("채팅 인입 (최근 기간)",
            f"{int(ch_cur):,}건",
    ...
    # 전년 동기 델타
    yoy_html = ""
    if ph_yoy is not None:
        yoy_html += (f"전화 전년동기 대비: "
                     f"{delta_row(ph_cur,None,ph_yoy,unit)}&nbsp;&nbsp;")
    if ch_yoy is not None:
        yoy_html += (f"채팅 전년동기 대비: "
                     f"{delta_row(ch_cur,None,ch_yoy,unit)}&nbsp;&nbsp;")
    if yoy_html:
        st.markdown(f"<div style='font-size:12px;margin-bottom:12px'>"
                    f"{yoy_html}</div>", unsafe_allow_html=True)
    ...
    for name, df in [("전화",phone),("채팅",chat),("게시판",board)]:
        tdf = df[df["일자"] >= pd.Timestamp(cutoff)]
        grp = tdf.groupby(pc).apply(
            lambda g: round((~g["미응대"]).sum()/len(g)*100,1)
            if len(g) > 0 else 0
        ).reset_index()
        grp.columns = [pc,"응답률(%)"]
        grp["채널"] = name
...
def page_phone(phone, unit, month_range):
    ...
    rr_cur = response_rate(phone)

    aht_cur, aht_prev, aht_yoy, _ = get_period_kpi(
        responded, unit, val_col="AHT(초)", agg="mean")
    wt_cur, wt_prev, wt_yoy, _   = get_period_kpi(
        responded, unit, val_col="대기시간(초)", agg="mean")
    ...
    with c4: kpi("응답률",       f"{rr_cur}%", color="#3b82f6")
    with c5: kpi("평균 AHT",
                 fmt_sec(responded["AHT(초)"].mean()),
    ...
    # 증감 배지
    badges = []
    pd_ = calc_delta(cur, prev)
    py_ = calc_delta(cur, yoy)
    if pd_ is not None:
        badges.append(delta_row(cur, prev, None, unit))
    if py_ is not None:
        badges.append(delta_row(cur, None, yoy, unit))
    if badges:
        st.markdown(
            f"<div style='font-size:12px;margin-bottom:16px'>"
            f"인입 &nbsp; {'&nbsp;&nbsp;'.join(b for b in badges if b)}</div>",
            unsafe_allow_html=True)
    ...
    with t2:
        ...
        if "인입시각" in phone.columns:
            ...
            hour_d = phone2.groupby(["시간대","미응대"]).size().reset_index(name="건수")
            ...
            aht_h = (responded.copy()
                     .assign(시간대=pd.to_datetime(
                         responded["인입시각"],format="%H:%M:%S",errors="coerce").dt.hour)
                     .groupby("시간대")[["대기시간(초)","AHT(초)"]]
                     .mean().round(1).reset_index())
            fig2 = go.Figure()
            fig2.add_trace(go.Bar(x=aht_h["시간대"],y=aht_h["대기시간(초)"],
                                  name="평균 대기(초)",marker_color="#3b82f6",opacity=0.8))
            fig2.add_trace(go.Scatter(x=aht_h["시간대"],y=aht_h["AHT(초)"],
    ...
    with t3:
        ...
        cutoff = get_chart_range(unit, month_range)
        pc     = get_period_col(unit)
        cats   = (phone[phone["대분류"]!=""]["대분류"].value_counts()
                  .head(6).index.tolist())
        fig5   = go.Figure()
...
def page_phone_agent(phone, unit, month_range):
    ...
    with t1:
        ...
        ag["응답률(%)"] = (
            ag["처리건수"]/(ag["처리건수"]+ag["미응대수"])*100).round(1)
    ...
    with t2:
        if "팀명" in phone.columns:
            ...
            cutoff = get_chart_range(unit, month_range)
            pc     = get_period_col(unit)
            teams  = team_r["팀명"].tolist()
            trend_dict = {}
            for tm in teams:
                trend_dict[tm] = responded[responded["팀명"]==tm]
            trend_chart(trend_dict, unit=unit,
                        val_col="AHT(초)", agg="mean",
...
def page_chat(chat, unit, month_range):
    ...
    rt_cur,rt_prev,rt_yoy,_ = get_period_kpi(
        responded, unit, val_col="응답시간(초)", agg="mean")
    ...
    with t2:
        ...
        hour_d = chat2.groupby(["시간대","미응대"]).size().reset_index(name="건수")
        hour_d["구분"] = hour_d["미응대"].map({True:"미응대",False:"응답"})
        fig = px.bar(hour_d,x="시간대",y="건수",color="구분",
                     barmode="stack",
...
def page_board(board, unit, month_range):
    ...
    with t3:
        ...
        with col2:
            ...
            fig5 = px.bar(plt_lt,y="플랫폼",x="근무내리드타임(분)",orientation="h",
...
def page_agent_total(phone, chat, board, unit, month_range):
    ...
    with t2:
        if all("근속그룹" in df.columns for df in [phone,chat,board]):
            ...
            sec(f"근속그룹별 처리건수 추이 ({unit})")
            trend_dict = {}
            for grp in tg_all["근속그룹"].tolist():
                trend_dict[grp] = pd.concat([
                    phone[~phone["미응대"] & (phone["근속그룹"]==grp)],
                    chat[~chat["미응대"] & (chat["근속그룹"]==grp)],
                    board[~board["미응대"] & (board["근속그룹"]==grp)],
                ])
            trend_chart(trend_dict,unit=unit,y_label="건수",
                        h=300,month_range=month_range)
...
def render_sidebar(phone, chat, board):
    with st.sidebar:
        ...
        all_brands = sorted(set(
            phone["브랜드"].dropna().unique().tolist() +
            chat["브랜드"].dropna().unique().tolist() +
            board["브랜드"].dropna().unique().tolist()
        ))
